...
QuadProgCommon::QuadProgCommon() : Q_(), C_(), B_(), X_(), fail_(0), iact_(), iter_(2) {}
...
void QuadProgCommon::fillQCBf(int nreq, int nrineq, const Ref<const MatrixXd>& Q, const Ref<const VectorXd>& C,
                              const Ref<const VectorXd>& Beq, const Ref<const VectorXd>& Bineq, bool isDecomp) {
  Q_ = Q;
  C_ = -C;
...
bool QuadProgDense::solve(const Ref<const MatrixXd>& Q, const Ref<const VectorXd>& C, const Ref<const MatrixXd>& Aeq,
                          const Ref<const VectorXd>& Beq, const Ref<const MatrixXd>& Aineq, const Ref<const VectorXd>& Bineq,
                          bool isDecomp) {
  ...
  qpgen2_(Q_.data(), C_.data(), &fddmat, &n, X_.data(), &crval, A_.data(), B_.data(), &fdamat, &q, &meq, iact_.data(), &nact, iter_.data(),
          work_.data(), &fail_);
...
bool QuadProgSparse::solve(const Ref<const MatrixXd>& Q, const Ref<const VectorXd>& C, const SparseMatrix<double>& Aeq,
                           const Ref<const VectorXd>& Beq, const SparseMatrix<double>& Aineq, const Ref<const VectorXd>& Bineq,
                           bool isDecomp) {
  int nrvar = static_cast<int>(C.rows());
  int nreq = static_cast<int>(Beq.rows());
  int nrineq = static_cast<int>(Bineq.rows());
  ...
  qpgen1_(Q_.data(), C_.data(), &fddmat, &n, X_.data(), &crval, A_.data(), iA_.data(), B_.data(), &fdamat, &q, &meq, iact_.data(), &nact,
